...
_COMMITTEES: list[CommitteeAlias] = [
    ...
    CommitteeAlias(
        ...
        CommitteeChamber.HOUSE,
        [
            "Federal Funding, Policy and Accountability",
            "House Committee on Federal Funding, Policy and Accountability",
        ],
    ),
    # Joint Committees
    ...
    CommitteeAlias(
        ...
        [
            ...
            "Senate Committee on Bills in the Third Reading",
            "Senate Bills in the Third Reading",
        ],
    ),
    CommitteeAlias(
        "S48",
...
def load_committee_registry_from_cache(cache: Cache) -> None:
    ...
    """
    try:
        committee_contacts: dict[str, dict] = cache.data.get("committee_contacts", {})
        for committee_id, info in committee_contacts.items():
            if committee_id not in COMMITTEE_REGISTRY:
                ...
                short_name = (
                    name.replace("Joint Committee on ", "")
                    .replace("House Committee on ", "")
                    .replace("Senate Committee on ", "")
                )
                variants = [short_name, name]
                committee = CommitteeAlias(committee_id, name, chamber, variants)
