...
def _pdf_contains_summary_for_bill(
        pdf_text: str, bill_id: str) -> Optional[str]:
...
def _download_hearing_pdfs(hearing_docs_url: str, base_url: str) -> list[dict]:
...
def _search_cached_pdfs_for_bill(
    ...
    for pdf_info in cached_pdfs:
        ...
        summary_snippet = _pdf_contains_summary_for_bill(pdf_text, bill_id)
        if summary_snippet:
            preview = (
...
def discover(base_url: str, bill: BillAtHearing) -> Optional[dict]:
    ...
    if hearing_docs_url not in _HEARING_PDF_CACHE:
        ...
        _HEARING_PDF_CACHE[hearing_docs_url] = _download_hearing_pdfs(
    ...
    return _search_cached_pdfs_for_bill(cached_pdfs, bill.bill_id)
