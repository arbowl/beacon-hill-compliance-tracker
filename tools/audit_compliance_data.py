...
OUTPUT_FOLDER = "../out/2025/12/13_mod"
...
def deduplicate_bills(committee_data: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
def calculate_global_stats(committee_data: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if total_bills > 0:
        overall_compliance_rate = round(
            ((compliant_bills + unknown_bills) / total_bills) * 100, 
            2
        )
    else:
...
def calculate_committee_stats(committee_id: str, bills: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
...
def display_global_stats(stats: Dict[str, Any]):
    ...
    print(f"├─ ✅ Compliant:        {stats['compliant_bills']} ({_percentage(stats['compliant_bills'], stats['total_bills'])}%)")
    print(f"├─ ❌ Non-Compliant:    {stats['non_compliant_bills']} ({_percentage(stats['non_compliant_bills'], stats['total_bills'])}%)")
    print(f"└─ ❓ Unknown:          {stats['unknown_bills']} ({_percentage(stats['unknown_bills'], stats['total_bills'])}%)")
    print()
...
def display_committee_stats(committee_id: str, stats: Dict[str, Any], committee_name: str = None):
...
def display_bill_details(bills: List[Dict[str, Any]], show_limit: int = 999):
    ...
    print(f"📄 Bill Details (showing first {min(show_limit, len(bills))} of {len(bills)}):")
    print("-" * 80)
    ...
    for i, bill in enumerate(bills[:show_limit], 1):
        ...
        print(f"   Hearing: {bill.get('hearing_date', 'N/A')} | Deadline: {bill.get('effective_deadline', 'N/A')}")
        print(f"   Reported Out: {'Yes' if bill.get('reported_out') else 'No'} | "
...
def interactive_menu(committee_data: Dict[str, Dict[str, Any]], global_stats: Dict[str, Any]):
    """
    ...
    while True:
        ...
        if choice == '0':
            ...
            for i, (committee_id, data) in enumerate(sorted(committee_data.items()), 1):
                bills = data.get('bills', [])
                stats = calculate_committee_stats(committee_id, bills)
                print(f"{i}. {committee_id}: {stats['total_bills']} bills, "
