...
try:
    ...
    # Try modern seaborn style, fall back to older style
    try:
...
def parse_report_out_compliance(reason: str) -> Optional[bool]:
    ...
    # Check for explicit non-compliance
    ...
    # If reason mentions "reported out" positively, assume compliant
...
def calculate_notice_compliance_score(notice_gap_days: Optional[int], 
                                     committee_type: str) -> float:
    """
    Calculate notice compliance as a percentage.
    For Joint: 10+ days = 100%, 5-9 = 50%, <5 = 0%
    For Senate: 5+ days = 100%, 2-4 = 50%, <2 = 0%
    For House: Always 100% (no requirement)
    """
    if notice_gap_days is None:
        return 0.0
    
    if committee_type.startswith('H'):
        return 100.0  # House has no notice requirement
    
    if committee_type.startswith('J'):
        if notice_gap_days >= 10:
            return 100.0
        elif notice_gap_days >= 5:
            return 50.0
        else:
            return 0.0
    
    if committee_type.startswith('S'):
        if notice_gap_days >= 5:
            return 100.0
        elif notice_gap_days >= 2:
            return 50.0
        else:
            return 0.0
...
def load_committee_data(json_dir: Path) -> Dict[str, Dict]:
    ...
    for json_file in json_dir.glob("basic_J*.json"):
        ...
        report_out_total = 0  # Only count bills where we can determine status
        
        bill_titles = []
        
        for bill in bills:
            # Skip bills without hearing dates (can't evaluate)
        ...
        avg_notice_days = notice_days_sum / bills_with_notice if bills_with_notice > 0 else 0
        ...
        # Calculate notice compliance score
        notice_compliance_pct = calculate_notice_compliance_score(
            int(avg_notice_days) if avg_notice_days > 0 else None,
            committee_id
        )
        ...
        committees[committee_id] = {
            ...
            'bills': bills  # Store full bill data for topic analysis
        }
...
def create_quadrant_matrix(committees: Dict[str, Dict], output_path: Path):
    ...
    # Calculate scores for each committee
    transparency_scores = []
    process_scores = []
    committee_ids = []
...
def extract_topic_keywords(title: str) -> List[str]:
    ...
    # If no topics found, try to extract first significant words
    if not found_topics:
        words = title_lower.split()[:3]
        found_topics = ['other']  # Generic category
...
def create_topic_analysis(committees: Dict[str, Dict], output_path: Path):
    ...
    # Calculate compliance rates per topic
    topic_stats = {}
    for topic, bills in topic_bills.items():
        if len(bills) < 3:  # Skip topics with too few bills
            continue
