...
def load_committee_data(json_dir: Path) -> Dict[str, Dict]:
    ...
    for json_file in json_dir.glob("basic_J*.json"):
        ...
        notice_compliance_pct = (
            (notice_in_range / notice_total * 100) if notice_total > 0 else 0
        )
        vote_compliance_pct = (
            (votes_present_count / total_bills * 100) if total_bills > 0 else 0
        )
        summary_compliance_pct = (
            (summary_present_count / total_bills * 100) if total_bills > 0 else 0
        )
        report_out_compliance_pct = (
            (report_out_compliant / report_out_total * 100)
            if report_out_total > 0
            else 0
        )
...
def create_quadrant_matrix(committees: Dict[str, Dict], output_path: Path):
    ...
    for cid, data in committees.items():
        transparency = (
            data["vote_compliance_pct"] + data["summary_compliance_pct"]
        ) / 2
        process = (
            data["notice_compliance_pct"] + data["report_out_compliance_pct"]
        ) / 2
    ...
    for i, cid in enumerate(committee_ids):
        ax.annotate(
            cid,
            (transparency_scores[i], process_scores[i]),
            xytext=(5, 5),
    ...
    ax.set_title(
        "Committee Compliance Typology Matrix", fontsize=16, fontweight="bold", pad=20
    )
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
...
def create_radar_charts(committees: Dict[str, Dict], output_path: Path):
    ...
    fig, axes = plt.subplots(
        rows, cols, figsize=(6 * cols, 6 * rows), subplot_kw=dict(projection="polar")
    )
    if n_committees == 1:
        axes = [axes]
    else:
    ...
    plt.suptitle(
        "Committee Compliance Profiles (Radar Charts)",
        fontsize=16,
        fontweight="bold",
        y=0.995,
    )
...
def create_heatmap(committees: Dict[str, Dict], output_path: Path):
    ...
    ax.set_title(
        "Compliance Gap Analysis by Committee", fontsize=14, fontweight="bold", pad=20
    )
...
def create_topic_analysis(committees: Dict[str, Dict], output_path: Path):
    ...
    for cid, data in committees.items():
        for bill in data["bills"]:
            ...
            for topic in topics:
                topic_bills[topic].append({"compliant": is_compliant, "title": title})
    ...
    sorted_topics = sorted(
        topic_stats.items(), key=lambda x: x[1]["rate"], reverse=True
    )
    ...
    bars = ax.barh(
        topics, rates, color=colors, alpha=0.7, edgecolor="black", linewidth=1.5
    )
    ...
    ax.set_title(
        "Compliance Rates by Bill Topic", fontsize=14, fontweight="bold", pad=20
    )
    ax.set_xlim(0, 100)
...
def main():
    ...
    create_quadrant_matrix(
        committees, output_dir / "visualization_1_quadrant_matrix.png"
    )
