...
class VoteRequirementRule(ComplianceRule):
    ...
    - Missing votes contribute to non-compliance (equivalent to
      _get_missing_requirements() lines 250-251)
    """
...
class SummaryRequirementRule(ComplianceRule):
    ...
    - Missing summaries contribute to non-compliance (equivalent to
      _get_missing_requirements() lines 252-253)
    """
...
if __name__ == "__main__":
    """Unit tests for the ruleset compliance pipeline."""
    from datetime import date, timedelta
    TEST_BACKWARDS_COMPATIBILITY = False
    if TEST_BACKWARDS_COMPATIBILITY:
        from components.compliance import classify as _classify

        print("Running ruleset unit tests...\n")

        def classify(*args, **kwargs):
            result = _classify(*args, **kwargs)
            if result.state == "Incomplete":
                result = BillCompliance(
                    result.bill_id,
                    result.committee_id,
                    result.hearing_date,
                    result.summary,
    ...
    def make_status(
        bill_id="H73",
        committee_id="J33",
        hearing_date=date(2025, 1, 15),
        announcement_date=date(2025, 1, 5),  # 10 days notice
        reported_out=True,
        reported_date=None,
        extension_until=None,
    ):
        deadline_60 = hearing_date + timedelta(days=60) if hearing_date else None
        deadline_90 = hearing_date + timedelta(days=90) if hearing_date else None
        return BillStatus(
            bill_id=bill_id,
            committee_id=committee_id,
    ...
    def make_summary(present=True):
        return SummaryInfo(
            present=present,
            location="hearing_pdf",
            source_url=None,
            parser_module=None,
        )

    def make_votes(present=True):
        return VoteInfo(
            present=present,
            location="bill_embedded",
            source_url=None,
            parser_module=None,
        )

    # Test 1: All requirements met (COMPLIANT)
    print("Test 1: All requirements met")
    status = make_status(
        reported_date=date(2025, 2, 1),  # Within deadline
    )
    summary = make_summary(present=True)
    votes = make_votes(present=True)
    result = classify("H73", "J33", status, summary, votes)
    assert result.state == ComplianceState.COMPLIANT, f"Expected COMPLIANT, got {result.state}"
    assert "All requirements met" in result.reason
    print("  ✓ PASSED")

    # Test 2: Missing votes (NON_COMPLIANT)
    print("Test 2: Missing votes")
    status = make_status(
        reported_date=date(2025, 2, 1),
    )
    summary = make_summary(present=True)
    votes = make_votes(present=False)
    result = classify("H73", "J33", status, summary, votes)
    assert result.state == ComplianceState.NON_COMPLIANT, f"Expected NON_COMPLIANT, got {result.state}"
    assert "no votes posted" in result.reason
    print("  ✓ PASSED")

    # Test 3: Missing summary (NON_COMPLIANT)
    print("Test 3: Missing summary")
    status = make_status(
        reported_date=date(2025, 2, 1),
    )
    summary = make_summary(present=False)
    votes = make_votes(present=True)
    result = classify("H73", "J33", status, summary, votes)
    assert result.state == ComplianceState.NON_COMPLIANT, f"Expected NON_COMPLIANT, got {result.state}"
    assert "no summaries posted" in result.reason
    print("  ✓ PASSED")

    # Test 4: Before deadline (UNKNOWN)
    print("Test 4: Before deadline")
    # Set hearing date in the future
    future_hearing = date.today() + timedelta(days=30)
    future_announcement = date.today() + timedelta(days=20)
    status = make_status(
        hearing_date=future_hearing,
        announcement_date=future_announcement,
        reported_out=False,
    )
    summary = make_summary(present=False)
    votes = make_votes(present=False)
    result = classify("H73", "J33", status, summary, votes)
    assert result.state == ComplianceState.UNKNOWN, f"Expected UNKNOWN, got {result.state}"
    assert "Before deadline" in result.reason
    print("  ✓ PASSED")

    # Test 5: Missing notice with no evidence (UNKNOWN)
    print("Test 5: Missing notice, no evidence")
    status = make_status(
        announcement_date=None,  # No announcement
        reported_out=False,
    )
    summary = make_summary(present=False)
    votes = make_votes(present=False)
    ...
    assert "no other evidence" in result.reason
    print("  ✓ PASSED")

    # Test 6: Missing notice with evidence (NON_COMPLIANT)
    print("Test 6: Missing notice, with evidence")
    status = make_status(
        announcement_date=None,  # No announcement
        reported_out=True,
    )
    ...
    print("Test 7: Insufficient notice (deal-breaker)")
    status = make_status(
        hearing_date=date(2025, 7, 15),
        announcement_date=date(2025, 7, 14),  # Only 1 day notice (need 10)
    )
    summary = make_summary(present=True)
    votes = make_votes(present=True)
    result = classify("H73", "J33", status, summary, votes)
    ...
    # Test 8: House committee (no notice requirement)
    print("Test 8: House committee (exempt from notice)")
    status = make_status(
        committee_id="H33",
        announcement_date=date(2025, 1, 14),  # Only 1 day notice (OK for House)
        reported_date=date(2025, 2, 1),
    )
    summary = make_summary(present=True)
    votes = make_votes(present=True)
    result = classify("H73", "H33", status, summary, votes)
    # Should be compliant even with short notice (House exempt)
    assert result.state == ComplianceState.COMPLIANT or result.state == ComplianceState.NON_COMPLIANT
    print("  ✓ PASSED")

    # Test 9: No hearing scheduled (UNKNOWN)
    print("Test 9: No hearing scheduled")
    status = make_status(
        hearing_date=None,
    )
    summary = make_summary(present=False)
    ...
    # Test 10: Senate bill in Joint committee
    print("Test 10: Senate bill in Joint committee")
    status = make_status(
        bill_id="S197",
        committee_id="J33",
        reported_date=date(2025, 2, 1),
    ...
    print("Test 11: Adequate notice")
    status = make_status(
        announcement_date=date(2025, 1, 5),  # 10 days before hearing
        reported_date=date(2025, 2, 1),
    )
    summary = make_summary(present=True)
    votes = make_votes(present=True)
    result = classify("H73", "J33", status, summary, votes)
    assert result.state == ComplianceState.COMPLIANT, f"Expected COMPLIANT, got {result.state}"
    assert "adequate notice" in result.reason or "All requirements met" in result.reason
    print("  ✓ PASSED")

    # Test 12: After deadline, missing requirements
    print("Test 12: After deadline, missing requirements")
    # Set hearing date in the past (deadline passed)
    past_hearing = date.today() - timedelta(days=100)
    past_announcement = past_hearing - timedelta(days=15)
    status = make_status(
        hearing_date=past_hearing,
        announcement_date=past_announcement,
        reported_out=False,
    )
    summary = make_summary(present=False)
    votes = make_votes(present=False)
