...
def format_bill_details_for_prompt(
    bill_ids: list[str],
    current_bills_by_id: dict[str, dict],
    include_hearing_date: bool = False
) -> str:
    """Format bill details for inclusion in LLM prompt.

    Args:
        bill_ids: List of bill IDs to format
        current_bills_by_id: Dictionary mapping bill_id to bill data
        include_hearing_date: Whether to include hearing date in format

    Returns:
        Formatted string with bill details, or "None" if empty
    """
    if not bill_ids:
        return "None"
    ...
    for bill_id in bill_ids:
        ...
        if include_hearing_date:
            hearing_date = bill.get("hearing_date", "N/A")
            details.append(
                f"{bill['bill_id']} | {bill['bill_title']} | "
                f"{hearing_date} | {bill['state']}"
            )
        else:
            details.append(
                f"{bill['bill_id']} | {bill['bill_title']} | {bill['state']}"
            )
...
def generate_llm_analysis(
    ...
    # Format bill details
    new_bills_details = format_bill_details_for_prompt(
        diff_report.get("new_bills", []), current_bills_by_id
    )
    bills_with_new_hearings_details = format_bill_details_for_prompt(
        diff_report.get("bills_with_new_hearings", []),
        current_bills_by_id,
        include_hearing_date=True
    )
    bills_reported_out_details = format_bill_details_for_prompt(
        diff_report.get("bills_reported_out", []), current_bills_by_id
    )
    bills_with_new_summaries_details = format_bill_details_for_prompt(
        diff_report.get("bills_with_new_summaries", []),
        current_bills_by_id
    )

    # Format compliance delta with sign
    compliance_delta = diff_report.get("compliance_delta", 0)
    compliance_delta_str = (
        f"{compliance_delta:+.1f}" if compliance_delta != 0
        else "0.0"
    )
    ...
    prompt = config.llm.diff_report_analysis_prompt.format(
        ...
        compliance_delta=compliance_delta_str,
        ...
        new_bills_details=new_bills_details,
        bills_with_new_hearings_details=bills_with_new_hearings_details,
        bills_reported_out_details=bills_reported_out_details,
        bills_with_new_summaries_details=bills_with_new_summaries_details,
    )
...
def generate_fallback_analysis(diff_report: dict) -> str:
    """Generate a simple text analysis when LLM is disabled.

    Args:
        diff_report: The diff report dictionary

    Returns:
        Simple analysis string
    """
    compliance_delta = diff_report.get("compliance_delta", 0)
    ...
    if compliance_delta > 0:
        trend = (
            f"Compliance rose by {compliance_delta:.1f} percentage points"
        )
    elif compliance_delta < 0:
        trend = (
            f"Compliance fell by {abs(compliance_delta):.1f} "
            "percentage points"
        )
    else:
        trend = "Compliance remained stable"

    activity_parts = []
    if new_bills_count > 0:
        activity_parts.append(f"{new_bills_count} new bill(s)")
    if bills_reported_out_count > 0:
        activity_parts.append(
            f"{bills_reported_out_count} bill(s) reported out"
        )
    if bills_with_new_summaries_count > 0:
        activity_parts.append(
            f"{bills_with_new_summaries_count} bill(s) with new summaries"
        )
    ...
    return (
        f"Over the last {time_interval}, {trend.lower()}. "
        f"Activity included{activity_str if activity_parts else ' no notable changes'}."  # noqa: E501
    )
